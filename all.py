...
class ErrorRecoveryEngine(models.AbstractModel):
    ...
    def _recover_data_error(self, sync_engine, error):
        """Recover from data error (signature matches callers)"""
        actions = []
        recommendations = []
        problematic_count = 0
        
        # Mark problematic media moves as error if unbalanced / no lines
        for media_move in sync_engine.media_move_ids:
            if not media_move.is_balanced or not media_move.line_ids:
                try:
                    media_move.write({'state': 'error', 'error_message': 'Data integrity issues detected'})
                    problematic_count += 1
                    actions.append(f'Isolated move {media_move.id}')
                except Exception:
                    # best effort
                    pass
        ...
        if problematic_count > 0:
            return {
                ...
                'recommendations': ['Review isolated media moves and fix mappings or balances'],
            }
